...
int insertSearchIntLt(INDEX_INFO* idxInfo, int value, int low, int high) {
    //printf("insertSearchIntLt, value %d, low %d, high %d\n", value, low, high);
    if (low > high) {
        return -1;
    }
    unsigned int lowValue = (idxInfo + low)->fieldValue.intValue;
    ...
    *matchCnt = 0;
    ...
    switch (opType) {
        ...
        case GT:
            {
                ...
                matchIdx1 = insertSearchIntLt(columnIdxInfo, value1, 0, g_totalLineCnt - 2);
        ...
        case LT:
            {
                ...
                matchIdx1 = insertSearchIntGt(columnIdxInfo, value1, 0, g_totalLineCnt - 2);
        ...
        case BT:
                ...
                matchIdx1 = insertSearchIntLt(columnIdxInfo, value1 - 1, 0, g_totalLineCnt - 2);
                // find the first item that's greater than or equal to value2
                matchIdx2 = insertSearchIntGt(columnIdxInfo, value2 + 1, 0, g_totalLineCnt - 2);
                if (-1 == matchIdx2) {
...
int insertSearchStrEqual(INDEX_INFO* idxInfo, char* value) {
    ...
    while (low <= high) {
        ...
        if(strcmp(midValue, value) > 0 ) {
            mmap_addr[end3] = tmpChar3;
            //return insertSearchStrEqual(idxInfo, value, low, mid - 1);
            high = mid - 1;
        }
        if(strcmp(midValue, value) < 0 ) {
            mmap_addr[end3] = tmpChar3;
            //return insertSearchStrEqual(idxInfo, value, mid + 1, high);
            low = mid + 1;
        }
    }
    return -1;
    ...
    *matchCnt = 0;
    ...
    switch (opType) {
        ...
        case GT:
            {
                // find the last item that little than value
                matchIdx1 = insertSearchStrLt(columnIdxInfo, value1, 0, g_totalLineCnt - 2);
                //printf("query greater than, last line little or equal: %d\n", matchIdx1);
                // not items are little than value
                if (-1 == matchIdx1) {
                    cnt = g_totalLineCnt - 1;
        ...
        case LT:
            {
                // find the first item that greater than or equal to value
                matchIdx1 = insertSearchStrGt(columnIdxInfo, value1, 0, g_totalLineCnt - 2);
                //printf("query little than, first line greater or equal: %d\n", matchIdx1);
                // not items are greater than value
                if (-1 == matchIdx1) {
                    cnt = g_totalLineCnt - 1;
        ...
        case BT:
                ...
                // find the first item that's greater than or equal to value2
                matchIdx2 = insertSearchStrGt(columnIdxInfo, value2, 0, g_totalLineCnt - 2);
                if (-1 == matchIdx2) {
